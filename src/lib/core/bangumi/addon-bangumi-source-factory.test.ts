...
} from "./addon-bangumi-source-factory";
...
describe("addon-bangumi-source-factory", () => {
  it("NEXT_PUBLIC_USE_ADDON_BANGUMI 未设置时默认 on", () => {
    expect(USE_ADDON_BANGUMI).toBe(true);
  });

  it("getAddonBangumiClient 懒加载单例，按 bangumi base 构造", () => {
    const first = getAddonBangumiClient();
    const second = getAddonBangumiClient();
