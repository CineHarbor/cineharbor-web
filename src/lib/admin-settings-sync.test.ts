...
describe('admin-settings-sync', () => {
  it('extracts only adminsettings business fields', () => {
    ...
    expect(snapshot).toEqual({
      SiteConfig: expect.objectContaining({ SiteName: 'Remote CineHarbor' }),
  ...
  it('redacts owner-only raw fields for admin role reads without hiding users', () => {
    ...
    expect(redacted.SiteConfig.SiteName).toBe('Remote CineHarbor');
  });
  ...
  it('applies only allowlisted fields when merging a sync snapshot', () => {
    ...
    remote.SiteConfig.SiteName = 'Desktop CineHarbor';
    ...
    expect(merged.SiteConfig.SiteName).toBe('Desktop CineHarbor');
