...
const DataMigration = ({ onRefreshConfig }: DataMigrationProps) => {
  ...
  const warningMessage = isDesktopLocalMigration
    ? '桌面本地模式下，仅迁移管理员配置和本地账号信息；播放记录、收藏、搜索历史与跳过片头片尾等浏览器本地数据不会导入或导出。'
    : '数据迁移操作请谨慎，确保已备份重要数据。';
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const filename = filenameMatch?.[1] || 'cineharbor-backup.dat';
  ...
  return (
    <>
      <div className='max-w-6xl mx-auto space-y-6'>
        ...
        {/* 主要操作区域 - 响应式布局 */}
        {isDesktopLocalMigration ? (
          <div className='rounded-lg border border-amber-200 bg-amber-50/60 p-4 text-sm text-amber-900 dark:border-amber-700 dark:bg-amber-900/10 dark:text-amber-100'>
            {warningMessage}
          </div>
        ) : null}
