...
jest.mock('@/lib/auth', () => ({
  BROWSER_AUTH_UPDATED_EVENT: 'cineharbor:browser-auth-updated',
...
jest.mock('@/lib/desktop/runtime-config', () => ({
  DESKTOP_RUNTIME_UPDATED_EVENT: 'cineharbor:desktop-runtime-updated',
}));
...
jest.mock('@/lib/player-enhancements', () => ({
  PLAYER_ENHANCEMENTS_UPDATED_EVENT: 'cineharbor:player-enhancements-updated',
...
describe('UserMenu', () => {
  ...
  it('refreshes the provider label after a desktop runtime event without status reads', async () => {
    ...
    act(() => {
      window.dispatchEvent(new Event('cineharbor:desktop-runtime-updated'));
    });
