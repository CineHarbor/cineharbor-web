...
try {
  ...
  // Establish origin without loading the React registration component, then seed an obsolete cache.
  ...
  assert.equal(
    initial.info.sha256,
    JSON.parse(originalInfo).sha256,
    'PWA served stale WASM metadata'
  );
  assert.equal(initial.legacy, false, 'Obsolete account cache was not removed');
  assert.equal(
    initial.privateCached,
    false,
    'Private API response entered cache'
  );
  assert.equal(
    initial.deniedStatus,
    401,
    'Unauthenticated API did not fail closed'
  );
  assert.equal(initial.hasWasm, true, 'WASM was not revision-precached');

  // Exercise a real installed Service Worker update using a changed public metadata revision.
  // This is PWA update evidence, not a substitute for the signed Desktop updater test.
  const marker = randomUUID();
  const changedInfo = JSON.stringify({
    ...JSON.parse(originalInfo),
    pwaUpdateSmoke: marker,
  });
  const revision = createHash('md5').update(changedInfo).digest('hex');
  const pattern = /\{url:"\/wasm\/build-info\.json",revision:"[a-f0-9]+"\}/g;
  assert.equal(
    (originalSw.match(pattern) || []).length,
    1,
    'Expected one revisioned WASM metadata entry'
  );
  const nextSw =
    originalSw.replace(
      pattern,
      `{url:"/wasm/build-info.json",revision:"${revision}"}`
  ...
  await evaluate(
    `(() => { const button = Array.from(document.querySelectorAll('button')).find(b => b.textContent.includes('更新并重新载入')); if (!button) throw new Error('Missing update action'); button.click(); return true; })()`
  );
  await reloaded;
  ...
  assert.equal(
    updated.info.pwaUpdateSmoke,
    marker,
    'New precache revision was not activated'
  );
  assert.equal(
    updated.stored,
    'stored-before-update',
    'IndexedDB was lost during PWA upgrade'
  );
  assert.equal(
    updated.local,
    'preserved',
    'User settings were lost during PWA upgrade'
  );
  const infoUrls = updated.cacheUrls.filter(
    (url) => new URL(url).pathname === '/wasm/build-info.json'
  );
  assert.equal(infoUrls.length, 1, 'Stale metadata revision remains cached');
  assert.equal(
    new URL(infoUrls[0]).searchParams.get('__WB_REVISION__'),
    revision
  );
  console.log(
    'PWA_PRODUCTION_RESULT=' +
      JSON.stringify({
        registration: true,
        accountCacheIsolation: true,
        wasmPrecache: true,
        oldCacheRemoval: true,
        realWaitingUpdate: true,
        staleRevisionRemoved: true,
        indexedDbPreserved: true,
        localSettingsPreserved: true,
      })
  );
} finally {
