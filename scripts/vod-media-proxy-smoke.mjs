...
async function main() {
  ...
  writeFileSync(
    configPath,
    JSON.stringify({
      sites: [{ key: 'mock', name: 'MockSite', api: 'http://127.0.0.1:1/api' }],
    }),
    'utf8'
  );
  ...
  try {
    ...
    const m3u8Url = `${ADDON_BASE}/media/vod/m3u8?source=mock&url=${encodeURIComponent(
      upstreamM3u8
    )}`;
    ...
    const result = {
      ...
      keyRewritten: Boolean(
        keyUrl &&
          keyUrl.includes('/media/vod/key') &&
          keyUrl.includes('source=mock')
      ),
